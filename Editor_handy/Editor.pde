...
void keyPressed() {
  ...
  if(key=='x'){
    int op1 = solve(list.get(look = solve(look)))%list.size(); 
    int op2 = solve(list.get(look = (look+1) % (list.size())))%list.size();
    int op3 = solve(list.get(look = (look+1) % (list.size())))%list.size();
    int op4 = solve(list.get(look = (look+1) % (list.size())))%list.size();
    int data1 = solve(list.get(solve(op1)))%list.size();
    int data2 = solve(list.get(solve(op2)))%list.size();
    //System.out.println("index:"+i+"op1:"+op1+"op2:"+op2+"op3:"+op3+"op4:"+op4+"data1:"+data1+"data2:"+data2);
    look = data1-data2 >= 0 ? (look+1) % (list.size()) : op4;
    list.set(op3,((data1-data2+list.size()) % (list.size()))*(list.get(solve(op1))<0?-1:1)+(list.get(solve(op1))<0?-1:0));
  }
  draw_list();
  //print(keyCode);
}
